...
class SessionLifecycle {
  ...
  async acquire() {
    ...
    const result = await this.registry.acquire(
      ...
      async () => {
        ...
        return { id: created.id, connectUrl: this.client.resolveConnectUrl(created) };
      },
  ...
  async ready() {
    if (this.connectUrl) return this.connectUrl;
    if (!this.sessionId) await this.acquire();
    if (this.connectUrl) return this.connectUrl;
    if (!this.readyPromise) {
      this.readyPromise = this.client
        .waitForRunning(this.sessionId)
        .then((session) => {
          this.connectUrl = this.client.resolveConnectUrl(session);
          this.log(`[service] session ${this.sessionId} is ready for key "${this.key}"`);
